...
Example:
  ...
  poker analyze packages/hand-history/fixtures/pluribus/pluribus_100.txt --hand 100000 --hero MrBlue --review hand-100000.review.json
`);
}
...
async function runAnalyze(): Promise<void> {
  ...
  const heroPlayerId =
    args[heroFlagIndex + 1];

  if (
    heroPlayerId === undefined
  ) {
    console.error(
      "Missing hero player ID",
    );
  ...
  if (
    ...
    console.log(
      `Review exported: ${resolvedReviewPath}`,
    );
  }
}
...
function printAnalysis(
  result: ReturnType<
    typeof analyzeHandHistory
  >,
  hand: ReturnType<
    typeof parsePluribusHand
  >,
): void {
  console.log(
    "Decision Analysis",
